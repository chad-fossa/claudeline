...
- **`scripts/test.sh`**: Extended with cases for file-first token resolution, `refresh_token_grant` (200 with/without refresh-token rotation, 500 leaves file/`.bak` untouched, `ACCOUNT_ASSUMED` refusal, lock contention), the capture script's identity probe (verified match / verified mismatch / probe-unreachable), auto-capture's trigger and two-sided veto window, and verified-identity marker rendering (118 cases total, 31 added for identity verification + auto-capture).
